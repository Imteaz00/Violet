...
export default function ProductCard({ product }: { product: ProductType }) {
    return (
        <div className="shadow-lg rounded-lg overflow-hidden bg-card relative
            ...
            <div className="flex flex-col gap-3 p-4">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-sm text-card-foreground">{formatCurrency(product.noOfShares > 0 ? Math.ceil(product.askingPrice / product.noOfShares) : product.askingPrice)}</p>
                        <p className="text-xs text-muted-foreground">per share</p>
                    </div>
                    <div>
                        <p className="text-xs text-muted-foreground">Remaining</p>
                        <p className="text-xs text-muted-foreground">Shares: <span className="text-accent-foreground text-sm">{product.remainingShares}</span> / {product.noOfShares}</p>
                    </div>
                </div>
                <Link href={`/products/${product.id}`}>
