...
import { ArrowRight, CircleDollarSign, Trash2 } from "lucide-react"
    ...
    return (
        <div className="flex flex-col gap-8 items-center justify-center mt-12">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
                {steps.map((step) => (
                    <div
                        className={`flex items-center gap-2 border-b-2 pb-4 ${step.id === activeStep ? "border-accent" : "border-muted"
                            }`}
                        key={step.id}
                    >
                        <div
                            className={`w-6 h-6 rounded-full p-4 flex items-center justify-center ${step.id === activeStep ? "bg-accent" : "bg-muted"
                                }`}
                        >
                            {step.id}
                        </div>
                        <p
                            className={`text-sm font-medium ${step.id === activeStep ? "text-accent-foreground" : "text-muted-foreground"
                                }`}
                        >
                            {step.name}
                        </p>
            ...
            <div className="w-full flex flex-col lg:flex-row gap-16">
                <div className={`w-full shadow-lg border border-border bg-card p-8 rounded-lg flex flex-col gap-8 ${bag.length !== 0 ? "lg:w-7/12" : ""}`}>
                        ...
                        activeStep === 2 ? (
                            ...
                            (
                                <p className="text-sm text-destructive">
                                    Please fill in shipping address continue.
                                </p>
                            )}
                </div>
                {bag.length !== 0 && <div className="w-full lg:w-5/12 shadow-lg border border-border bg-card p-8 rounded-lg flex flex-col gap-8 h-max">
